...
## MVP 1.0 – Trade history at scale 🚧 In progress

Closes a gap PR #125 explicitly deferred: `GET_TRADE_HISTORY` loads the entire `trades` table, unbounded, on every connection's connect, and the gateway relays it as one WebSocket frame - unsustainable once trade volume grows into the thousands or millions of rows. Moves filtering, sorting, and pagination server-side, using cursor/keyset pagination (not offset/page-number, for correctness under concurrent inserts and to avoid `OFFSET` performance decay at scale) and AG Grid Community's Infinite Row Model (confirmed Community-tier, not Enterprise).
