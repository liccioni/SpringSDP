...
* `KNOWN_SYMBOLS` in the frontend duplicates `MarketDataService`'s tradable symbol set on the backend, with no symbol-discovery message to keep them in sync. If the backend's tradable symbols ever change, the frontend constant has to be updated by hand or the grid will silently miss a symbol. Flagged in a comment at the point of duplication; adding a symbol-discovery message was considered out of scope for this issue. Resolved in [ADR 0029](0029-shared-symbol-catalog.md) (issue #159): a shared `contracts.SymbolCatalog` constant and a new `SYMBOLS` discovery message replace the hardcoded frontend copy (and two other independent backend copies found along the way). This ADR's own "nothing until subscribed" decision is unaffected — the frontend now subscribes to a fixed default subset instead of everything, still via real `SUBSCRIBE` traffic, not a change to the transport-level default itself.
