...
4. **`cellFlashDuration`/`cellFadeDuration` must be tuned to the data's own update cadence.** `PriceGrid` uses `enableCellChangeFlash` (themed via `frontend/src/theme/tradingTheme.ts`, an explicit AG Grid Theming API theme rather than the implicit default) to flash a cell when its tick changes. AG Grid's defaults flash-and-fade over roughly 2 seconds combined; `MarketDataService` ticks every 1 second, so the default timing never let a flash finish before the next one re-triggered it — cells looked permanently highlighted instead of flashing. Set both durations (150ms flash / 350ms fade) short enough to settle well within one tick interval.
...
- AG Grid's Theming API and modular architecture have changed significantly across major versions; the registration and theming approach recorded here (including the shared `tradingTheme`) may need revisiting on a future AG Grid upgrade.
