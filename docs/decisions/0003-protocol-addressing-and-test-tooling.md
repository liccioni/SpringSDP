...
* **Frontend addressing**: the frontend reads the WebSocket URL from a `VITE_WS_URL` environment variable, defaulting to `ws://localhost:8080/ws` for local `npm run dev`. No Vite dev-server proxy — the frontend connects to the backend directly. **Correction (see ADR 0006):** this same `ws://localhost:8080/ws` default is also correct under Docker Compose — the *browser*, not the frontend container, opens the WebSocket connection, so it needs the backend's host-published port, not a Docker-internal service name. The original text here (Compose "supplies its own value pointing at the backend service's container address") was wrong and is superseded by ADR 0006.
