...
**Update (MVP 0.7 retro):** two environment-state gotchas found re-verifying #94 on a machine that had an already-running dev stack from before the merge - neither is a code or design defect, both are specific to crossing this one "a whole service gets deleted" boundary:

* **A `docker compose up` left over from before #94 keeps the old `backend` container running as an orphan** - it's no longer in `docker-compose.yml`, so Compose won't touch it on a plain `up`, and it keeps squatting on port 8080 (now `gateway`'s port, per the bullet above) until removed explicitly (`docker compose up --remove-orphans`, or a `docker compose down` first). `docker compose up` warns about this ("Found orphan containers...") but doesn't fail or auto-clean, so it's easy to miss and end up debugging why the new `gateway` container never seems to be the one actually answering requests.
* **A `gateway` container already running from before this merge keeps serving its old image indefinitely, even across `docker compose restart` / `up -d --force-recreate`** - recreating a container reuses whatever image is already tagged locally; it doesn't rebuild from source. Only `./startUpDocker.sh` (or a manual `./gradlew jibDockerBuild` per service, per `README.md`'s Docker Compose section) produces an image reflecting this issue's `GatewayApplication` package move - already documented there, but easy to skip when the immediate goal is "just restart the container" rather than "pick up a source change." Symptom was subtle: the container logged a clean startup with no errors, just silently missing every bean `com.sdp.config`'s sibling-package scan should have picked up (see this ADR's #91 update) - because it was still running the pre-#94 package layout that gotcha describes.
