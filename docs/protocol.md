...
`LOGOUT` (issue #102, [ADR 0023](decisions/0023-oidc-rp-initiated-logout.md)) fires from `AuditingLogoutSuccessHandler`, once `POST /logout` completes and before the browser is redirected to Keycloak's end-session endpoint. Like `LOGIN_SUCCESS`/`LOGIN_ERROR` below, it carries a `null` session id: the security layer has no access to the app's own connection-scoped `Session` (ADR 0017), only the HTTP-level `Authentication`. ADR 0019's decision not to record a `SESSION_ENDED` event or hook WebSocket disconnect still holds — logout is a browser-driven HTTP flow, not a WebSocket lifecycle change.
