...
* TRADE_CREATED — payload: a `Trade`, with the same `id` as the `PendingTrade` it was confirmed from, carrying `submittedBy` (the username that submitted it). Sent in reply to `CONFIRM_TRADE`, delivered only to the submitting session (issue #152, [ADR 0028](decisions/0028-per-session-trade-delivery.md)) — never broadcast.
...
* TRADE_REJECTED — payload: a `TradeRejected` (`symbol`, `side`, `price`, `quantity`, `reason`, `submittedBy`). Sent in reply to an invalid `CREATE_TRADE`, delivered only to the submitting session (issue #152, [ADR 0028](decisions/0028-per-session-trade-delivery.md)) — never broadcast.
...
* CREATE_TRADE — no longer executes immediately (see ADR 0018): validates and prices the trade, then replies with `TRADE_PENDING` holding it for confirmation. An invalid request still replies with `TRADE_REJECTED`, delivered only to the submitting session (issue #152, ADR 0028).
* CONFIRM_TRADE — payload `{ "id": "<pending trade id>" }`. Executes a previously requested pending trade, triggering `TRADE_CREATED`, delivered only to the submitting session (issue #152, ADR 0028). An unknown or already-resolved id is a silent no-op.
...
`TRADE_CREATED` and `TRADE_REJECTED` are delivered only to the session that submitted the trade, not broadcast (issue #152, [ADR 0028](decisions/0028-per-session-trade-delivery.md)) — every other trader used to see every trade execution and rejection, deliberately deferred as a gap when `Trade`/`TradeRejected` still carried no submitter identity (see [ADR 0017](decisions/0017-session-scope.md)). `Trade`/`TradeRejected` now carry `submittedBy` (the username from the originating `TradeCommand`, not persisted on the `trades` table itself), and `SdpWebSocketHandler` filters `EventBus` delivery by it the same way `SymbolSubscription` filters `PRICE_TICK` by subscribed symbol.
...
`TRADE_HISTORY`, `TRADE_PENDING`, and `TRADE_CANCELLED` are neither broadcast nor `EventBus`-filtered: each is a direct reply to one connection's own request (`GET_TRADE_HISTORY`, `CREATE_TRADE`, `CANCEL_TRADE` respectively), delivered through that connection's own per-connection sink rather than the shared `EventBus`. Unlike `PRICE_TICK`'s subscription state or `TRADE_CREATED`/`TRADE_REJECTED`'s submitter filtering, this isn't connection-local *filtering* of a shared stream — no other connection ever sees one of these messages that wasn't theirs to begin with. `TRADE_CREATED` (sent in reply to `CONFIRM_TRADE`) used to be the odd one out here: even though confirmation is itself a targeted, one-connection request, its result was still broadcast to everyone from before the two-step workflow existed (see [ADR 0018](decisions/0018-two-step-trade-confirmation.md)) until issue #152/ADR 0028 closed that gap.
