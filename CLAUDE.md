...
MVP 0.1 through MVP 1.3 are done (see [retro 0006](docs/retros/0006-mvp-0.6.md) for MVP 0.6, [retro 0007](docs/retros/0007-mvp-0.7.md) for MVP 0.7, [retro 0008](docs/retros/0008-mvp-0.8.md) for MVP 0.8, [retro 0009](docs/retros/0009-mvp-0.9.md) for MVP 0.9, [retro 0010](docs/retros/0010-mvp-1.0.md) for MVP 1.0, [retro 0011](docs/retros/0011-mvp-1.1.md) for MVP 1.1, [retro 0012](docs/retros/0012-mvp-1.2.md) for MVP 1.2, and [retro 0013](docs/retros/0013-mvp-1.3.md) for MVP 1.3). No milestone is currently in progress; see [roadmap.md](docs/roadmap.md)'s "What's next" for the unscheduled backlog. MVP 0.6 (identity & session — Keycloak, Spring Security, Redis-backed Spring Session) reversed this file's Core philosophy section's "no framework auth" and "no Redis" constraints the same deliberate way "no authentication" was lifted for MVP 0.5. MVP 0.7 (service topology & messaging) reversed "no microservices" the same way: the monolith split into a WebSocket gateway (`gateway/`), a market data service, and a backend/trading service over RabbitMQ (#89-#93), and the original monolithic `backend/` module was deleted once every flow had migrated off it (#94). MVP 0.8 (session lifecycle & cleanup) closed the two gaps left over from earlier milestones: a real logout flow (#102, [ADR 0023](docs/decisions/0023-oidc-rp-initiated-logout.md)) and automatic cleanup of pending trades left dangling on disconnect (#79, [ADR 0024](docs/decisions/0024-cancel-pending-trades-on-disconnect.md)). MVP 0.9 (role enforcement & blotter usability) closed the last gap: the `trader`/`viewer` Keycloak roles defined since MVP 0.6 were never actually enforced anywhere in the stack (#117, [ADR 0025](docs/decisions/0025-trader-role-enforcement.md)), and the trade blotter had no filtering or pagination as trade volume grows (#118). MVP 1.0 (trade history at scale) closed the gap PR #125 explicitly deferred: `GET_TRADE_HISTORY` loaded the entire `trades` table unbounded on every connect, so this milestone moved filtering, sorting, and pagination server-side via cursor/keyset pagination and AG Grid Community's Infinite Row Model (#130-#133, [ADR 0026](docs/decisions/0026-cursor-paginated-trade-history.md)), plus two of the three bugs found live-verifying MVP 0.9 (#126 and #127). MVP 1.1 (session resilience) closed the third of those bugs, #128 (a gateway process swap with a live Redis session could enter an infinite OAuth2 redirect loop): every component used to open its own WebSocket, turning a rare Redis-session race into a near-certainty on every page load, so the frontend now shares a single connection app-wide and retries a handshake that closes before opening a bounded number of times before falling back to the Keycloak login redirect ([ADR 0027](docs/decisions/0027-single-shared-socket-with-bounded-retry.md)) — a mitigation of the trigger and the symptom, not a fix inside `ReactiveRedisSessionRepository` itself, which remains unconfirmed. MVP 1.2 (per-session trade delivery) closed #152, found while root-causing #128 (unrelated to that bug's actual cause): the gateway's `EventBus` broadcast `TRADE_CREATED`/`TRADE_REJECTED` to every connected session regardless of who submitted the trade — a gap [protocol.md](docs/protocol.md)'s Broadcast semantics section already flagged as deferred. What shipped is more precise than "scope delivery to the submitting session only": the submitter's own acknowledgment and the trade blotter's live cross-user refresh are genuinely different concerns, so `CONFIRM_TRADE` now gets a correlated reply (delivered regardless of subscription state) while `TRADE_CREATED`'s blotter-refresh copy follows a new, explicit `SUBSCRIBE_BLOTTER` subscription mirroring `PRICE_TICK`'s existing model — not blanket broadcast, not submitter-only ([ADR 0028](docs/decisions/0028-scoped-trade-delivery.md)). A related idea raised during this milestone's design discussion — expanding price-tick symbols with a default-subscribed subset — was deliberately scoped out at the time rather than bundled in; it shipped as MVP 1.3, below. MVP 1.3 (symbol catalog & default subscriptions) closed #159: it replaced three independently hardcoded tradable-symbol lists (`market-data-service`'s seed prices, `trading-service`'s validation set, and the frontend's `PriceGrid`) — a fourth, previously unflagged copy in a gateway test double was found and fixed along the way — with a single `contracts.SymbolCatalog` constant (14 pairs, up from 3), and moved the frontend off auto-subscribing to every known symbol toward a fixed "majors" (6-pair) default-subscribed subset delivered via a new `SYMBOLS` discovery message sent right after `HELLO`. The catalog is a compile-time shared constant rather than a runtime broadcast from `market-data-service`, deliberately: see [ADR 0029](docs/decisions/0029-shared-symbol-catalog.md) for why that avoids reintroducing the exact startup-ordering race [testing.md](docs/testing.md) already documents for `market-data-service`'s own `StreamBridge` publisher.
